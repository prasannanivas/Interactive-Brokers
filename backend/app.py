...
async def get_daily_snapshots_by_symbol(
    ...
    """
    Get per-day BULLISH/BEARISH signal counts for a single symbol, straight
    from the daily snapshot archive. Each snapshot is captured once a day
    directly from that symbol's watchlist buy_signals/sell_signals arrays —
    the exact same fields the live Currency Signal Matrix reads — so a given
    day's count here always matches what the Matrix showed on that day.

    Path params:
        symbol: Watchlist symbol, e.g. USDCAD

    Query params:
        days: Number of days back to retrieve (default: 90)
